...
#This lets you train:
#with no batch correction: sample_ids=None
#with batch correction: pass sample_ids

def total_loss(xs, outputs, sample_ids=None, lambda_corr=1.0, lambda_batch=1.0):
    ...
    if sample_ids is not None:
        batch = batch_alignment_loss(outputs["z"], sample_ids)
...
def batch_alignment_loss(z, sample_ids, sigma=1.0):
    ...
    for i in range(len(unique_ids)):
        for j in range(i + 1, len(unique_ids)):
            zi = z[sample_ids == unique_ids[i]]
            zj = z[sample_ids == unique_ids[j]]
            loss = loss + mmd_loss(zi, zj, sigma=sigma)
            count += 1
